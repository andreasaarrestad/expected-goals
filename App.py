...
df_merged = transform_events(num_games=10)
...
tab2 = dbc.Card([dropdown, dbc.Tab(dbc.Tab(graph, label="2", tab_id="2"))])
...
def update_graph(dropdown_value):
    ...
    df = df_first[['posx', 'posy', 'goal', 'side', 'minutes', 'quarter', 'distance', 'angle', 'header', 'penalty', 'shot']]
...
def create_table(df):
    ...
    df = pd.DataFrame({'': ['xG', 'Goals'],df['home_team'].values[0]: [xg_team1, goals_team1], df['away_team'].values[1]: [xg_team2, goals_team2]}, index=['xG', 'Goals'])
